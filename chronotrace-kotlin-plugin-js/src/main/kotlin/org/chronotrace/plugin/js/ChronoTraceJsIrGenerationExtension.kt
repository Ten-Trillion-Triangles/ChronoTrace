...
class ChronoTraceJsIrGenerationExtension : IrGenerationExtension {
    override fun generate(moduleFragment: IrModuleFragment, pluginContext: IrPluginContext) {
        ...
        println("ChronoTrace[JS]: $instrumentedFns functions instrumented, $capturedLocals locals captured")
        classStats.entries.take(5).forEach { (cls, fns) ->
            println("ChronoTrace[JS]:   $cls → ${fns.size} fn(s) instrumented (${fns.take(3).joinToString()})")
        }
    }
}
